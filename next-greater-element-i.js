...
var nextGreaterElement = function(findNums, nums) {
    ...
    let elements = {};
    for (let i = 0; i < nums.length; i++) elements[nums[i]] = nums.slice(i);

    for (let i = 0; i < findNums.length; i++) {
        let closest = -1;

        for (let j = 0; j < elements[findNums[i]].length; j++) {
            if (elements[findNums[i]][j] > findNums[i]) {
                closest = elements[findNums[i]][j]
                break;
            }
        }
